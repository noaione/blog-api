...
function getImageStaggered() {
	const index = calculateWallpaperIndex();
	return IMAGE_LIST[index];
}

function getCurrentSplashImage(imgIndex: number | null) {
	if (typeof imgIndex === 'number' && !Number.isNaN(imgIndex) && imgIndex >= 0 && imgIndex < IMAGE_LIST.length) {
		return IMAGE_LIST[imgIndex];
	}

	// Randomize
	return getImageStaggered();
}

export { getCurrentSplashImage }
